...
![1](https://github.com/user-attachments/assets/1234e584-d83c-4829-bca5-2050a4884993)

图1 展示了谷歌学术上关于具身AI主题的搜索结果。垂直轴和水平轴分别表示出版物数量和年份。自2023年MLMs的突破以来，出版物呈指数级增长。
![2](https://github.com/user-attachments/assets/ff6429f8-7227-48e6-92ca-4fd105df16f1)
...
![3](https://github.com/user-attachments/assets/37854d4d-89f0-4aa0-8c16-5186faf3b24e)
...
![4](https://github.com/user-attachments/assets/42c7eaa0-4840-422a-b291-68b890bf24e5)
...
![5](https://github.com/user-attachments/assets/fd9e4409-b027-4cfd-ae9d-442bdf6f40bc)
...
![6](https://github.com/user-attachments/assets/25ed643b-74bd-4179-a45e-72264cdb12de)
![7](https://github.com/user-attachments/assets/435c0e7c-888a-48b4-ac7f-f170913e6bec)
![8](https://github.com/user-attachments/assets/e66d5215-99b2-4a89-ac5f-286cdde9f092)
...
![9](https://github.com/user-attachments/assets/a2a75918-e1f4-42b1-8f9c-4398f9f72560)
...
![10](https://github.com/user-attachments/assets/e8c72406-81cf-4751-9c48-d777684c1f94)
![11](https://github.com/user-attachments/assets/79635236-117e-4a15-82fc-969819c02efe)
...
![12](https://github.com/user-attachments/assets/1415e7b3-88fc-4329-be8e-12a945255389)
...
![13](https://github.com/user-attachments/assets/736b2261-d511-4428-bb09-ce02a231c547)
...
![14](https://github.com/user-attachments/assets/d768cd5e-a954-4574-acfe-6fbe7feebc74)
...
![15](https://github.com/user-attachments/assets/c1cae00b-1e46-4b26-8724-5290284b5702)
...
![16](https://github.com/user-attachments/assets/58b7d1c9-c330-4ae0-a9e5-c2425023fb39)
...
![17](https://github.com/user-attachments/assets/9e91db1c-96b9-4408-9327-9146dce3a33f)
...
![18](https://github.com/user-attachments/assets/7992fe32-ce9d-424c-a74f-c720b0b3ed55)
...
![19](https://github.com/user-attachments/assets/59444f7e-2024-4029-ab32-fa555b1c2db4)
...
![20](https://github.com/user-attachments/assets/bf302ed6-14c5-4928-bdf5-017e8eff4751)
...
**4) 局限性** ： a) 数据集：构建数据集需要大量人力和资源。此外大规模数据集仍然很少，不同数据集的模型性能评估指标也不尽相同。b) 模型：尽管 LLM 取得了进步，但这些模型的性能仍然明显落后于人类水平。未来的工作可能会更侧重于有效存储代理探索到的环境信息，并引导它们根据环境记忆和问题计划行动、同时增强其行动的可解释性。

### B. 具身抓取（Embodied Grasping）

除了与人类进行问答式互动外，具身交互还包括根据人类指令执行操作，如抓取和放置物体，从而完成与机器人、人类和物体之间的互动。机器人抓取需要全面的语义理解、场景感知、决策制定和稳健的控制规划。具身抓取方法将传统的机器人运动学抓取与大型模型如 LLMs [280] 和视觉语言基础模型 [14]，使机器人能在多感官感知下执行抓取任务，包括视觉主动感知、语言理解和推理。图 12(b) 展示了人-智能体-物体交互的概况，在这种情况下，智能体完成了抓取任务。
![21](https://github.com/user-attachments/assets/6cee7c69-ada3-42f7-a672-3dfadccfac07)

**1) 抓取器** ： 目前抓取技术的研究重点是双指并行抓手和五指灵巧手。对于双指并行抓取器，一般分为两类： 4-DOF 和 6-DOF [290]。4-DOF 抓取综合法 [281], [282], [286] 利用三维位置与自上而下的手的方向（偏航）来定义抓取，通常称为 "自上而下的抓取"。相比之下，6-DOF 抓取合成 [284],[291],[292] 通过六维位置和方向定义抓取姿势。对于五指灵巧手抓取器而言，ShadowHand 是一种广泛使用的五指机器人灵巧手，具有 26 个自由度 (DOF)。这种高维度大大增加了生成有效抓取姿势和规划执行轨迹的复杂性。

**2) 数据集** ： 最近，大量的抓取数据集[281]-[285]已经生成。这些数据集通常包含基于图像（RGB、深度）、点云或三维场景的抓取数据。随着多语言模型的出现以及基础语言模型在机器人抓取中的应用，迫切需要包含语言文本的数据集。因此，现有的数据集已进行了扩展或重建，以创建语义抓取数据集 [287]-[289],[293]。这些数据集有助于研究以语言为基础的抓取模型，使智能体能对语义广泛理解。

传统的抓取数据集包括单个物体 [281] 和杂乱场景 [286] 的数据，提供符合每个物体运动学的稳定抓取注释（4-DOF 或 6-DOF）。这些数据可以从真实桌面环境 [281] 收集，通常包括 RGB、深度和点云数据，也可以从虚拟环境 [284] 收集，包括图像数据、点云或场景模型。虽然这些数据集对抓取模型很有用，但它们缺乏语义信息。为了弥补这一缺陷，这些数据集已通过语义表达进行了增强或扩展 [287]、[294]，从而将语言、视觉和抓取联系起来。通过整合语义信息，智能体可以更好地理解和执行抓取任务。这种增强可以开发更复杂、语义意识更强的抓取模型，从而促进与环境更直观、更有效的互动。表 X 列出了上述数据集包括传统的抓取数据集和基于语言的抓取数据集。

**3) 语言引导抓取** ： 语言引导抓取的概念 [287], [288], [294] 是在整合的基础上发展起来的，它结合了多语言模型，为智能体提供了语义场景推理的能力。这样，智能体就能根据隐式或显式人类指令执行抓取操作。图 12 (c) 展示了近年来有关语言引导抓取主题的出版趋势。随着 LLM 的发展，研究人员对这一主题的兴趣与日俱增。目前，抓取研究越来越关注开放世界场景，强调开放集泛化 [295] 方法。通过利用 MLM 的泛化能力，机器人可以在开放世界环境中以更高的智能和效率执行抓取任务。

在语言引导的抓取过程中，语义可以来源于显式指令 [295], [296] 和隐式指令 [288], [289]。显性指令明确指出要抓取的物体的类别，如香蕉或苹果。而隐性指令则需要通过推理来确定要抓取的物体或物体的一部分，其中涉及空间推理和逻辑推理。

空间推理 [287] 是指指令中可能包含要抓取的物体或部件的空间关系，从而需要根据场景中物体的空间关系推断抓取姿势。例如，"抓住棕色纸巾盒右边的键盘"就涉及到对物体空间排列的理解和推断。逻辑推理 [288] 则涉及可能包含逻辑关系的指令，需要通过推理来辨别人类的意图，进而抓住目标。例如，"我渴了，你能给我点喝的吗？"会促使智能体可能递上一杯水或一瓶饮料。智能体必须确保液体在交接过程中不会溢出。在这两种情况下，语义的整合都是非常重要的。

在这两种情况下，语义理解与空间和逻辑推理的整合使智能体能够有效、准确地执行复杂的抓取任务。图 12 (a) 描述了各种类型的语言引导抓取任务。

**4) 端到端方法** ： CLIPORT [294] 是一种以语言为条件的模仿学习智能体，它将视觉语言预训练模型 CLIP 与 Transporter Net 相结合，为语义理解和抓取生成创建了一种端到端的双流架构。它利用从虚拟环境中收集的大量专家示范数据进行训练，使智能体能够执行语义引导的抓取。在 OCID 数据集的基础上，CROG [287] 提出了一个视觉语言抓取数据集，并引入了一个有竞争力的端到端基线。它利用 CLIP 的视觉基础功能，直接从图像-文本对中学习抓取合成。推理抓取 [288] 基于 GraspNet1 亿级数据集引入了首个推理抓取基准数据集，并提出了端到端推理抓取模型。该模型集成了多模态 LLM 和基于视觉的机器人抓取框架，可根据语义和视觉生成抓取动作。SemGrasp [289] 是一种基于语义的抓取生成方法，它将语义信息纳入抓取表征，以生成灵巧的手部抓取姿势。它引入了一种离散表示法将抓握空间与语义空间对齐，从而能够根据语言指令生成抓握姿势。为了便于训练，我们提出了一个大规模的抓握-文本对齐数据集 CapGrasp。

**5) 模块化方法** ： F3RM [295] 试图将 CLIP 的文本图像先验提升到三维空间，利用提取的特征进行语言定位，然后生成抓取点。它 精确的三维几何与二维基础模型的丰富语义结合起来，利用提取的特征进行语言定位，然后生成抓取点。基础模型的丰富语义相结合，利用从 CLIP 中提取的特征，通过自由文本自然语言指定操作对象。它展示了对未见过的表达和新对象类别的能力。GaussianGrasper[296］利用三维高斯场实现语言引导的抓取任务。所提出的方法首先是构建三维高斯场，然后进行特征提炼。随后，利用提取的特征进行基于语言的定位。最后，根据 SOTA 预训练抓取网络生成抓取姿势[297]。它将开放式词汇语义与精确的几何形状整合在一起、 实现基于语言指令的抓取。

这些方法通过利用端到端和模块化框架，推动了语言引导抓取领域的发展，从而增强了机器人通过自然语言指令理解和执行复杂抓取任务的能力。具身抓取使机器人能够与物体互动，从而提高机器人的智能和在家庭服务和工业制造领域的实用性。然而，现有的具身抓取方法有其局限性，例如依赖大量数据，以及对未见数据的普适性较差。未来的研究将侧重于提高智能体的通用性，使机器人能够理解更复杂的语义，抓取更多种类的未知物体，并完成复杂的抓取任务。

## VI. 具身智能体
智能体被定义为能够感知其环境并采取行动以实现特定目标的自主实体。最近在多模态大型模型（MLMs）方面的进展进一步扩展了智能体在实际场景中的应用。当这些基于MLM的智能体被具身化到物理实体中时，它们能够有效地将它们的能力从虚拟空间转移到物理世界，从而成为具身智能体\[298\]。图13显示了具身智能体的时间线概述。为了使具身智能体能够在信息丰富且复杂的现实世界中操作，具身智能体已经被开发出强大的多模态感知、交互和规划能力，如图14所示。为了完成一个任务，具身智能体通常涉及以下过程：1）将抽象和复杂的任务分解为特定的子任务，这被称为高水平的具身任务规划。2）通过有效利用具身感知和具身交互模型或利用基础模型的策略功能逐步实施这些子任务，称为低水平的具身动作规划。值得注意的是，任务规划涉及行动前的思考，因此通常被认为是在网络空间中进行的。相比之下，动作规划必须考虑与环境的有效交互以及将这些信息反馈给任务规划者以调整任务规划。因此，对具身智能体来说，将其能力从网络空间对齐和概括到物理世界至关重要。
![22](https://github.com/user-attachments/assets/fd28ea25-3707-4e2a-bb69-f3e73d819938)
![23](https://github.com/user-attachments/assets/5d93806e-b01f-46e3-a89e-0098acb17443)
...
![26](https://github.com/user-attachments/assets/86b078af-e4b2-4f1d-a315-90e38ca55290)
![27](https://github.com/user-attachments/assets/9c5be499-e20b-43a8-af8d-ea526ced09db)
...
![28](https://github.com/user-attachments/assets/ec0175de-b8e0-4658-8d33-758cd7cce724)
![29](https://github.com/user-attachments/assets/1f1a9581-80e8-4595-987d-f8b9af22fb56)
...
![30](https://github.com/user-attachments/assets/b6b408cc-be55-44b3-92a1-7e38811f7e5b)
...
标签：具身人工智能，多模态大型模型，世界模型，智能体，机器人学 <br>
概述：本调查研究了具身AI的最新进展，探讨了多模态大型模型和世界模型在实现具身智能体中的重要作用。 <br>
